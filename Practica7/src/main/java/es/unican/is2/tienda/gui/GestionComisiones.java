	...
	public static void main(String[] args) {//1
		...
		while (true) {//1 CC+1
			...
			case NUEVA_VENTA:
				...
				try {
					if (!tienda.anhadeVenta(dni, importe)) {//1 CC+3
						mensaje("ERROR", "El vendedor no existe");
					}
				} catch (IOException e) {//1 CC+3
			...
			case VENDEDOR_DEL_MES:
				...
				for (Vendedor v : vendedores) {//1 CC +3
					if (v.getTotalVentas() > maxVentas) {//1 CC+4
						maxVentas = v.getTotalVentas();
						resultado.clear();
						resultado.add(v);
					} else if (v.getTotalVentas() == maxVentas) {// CC+5
						resultado.add(v);
					}
				}

				msj = "";
				for (Vendedor vn : resultado) {//1 CC+3
					msj += vn.getNombre() + "\n";
				}
			...
			case VENDEDORES:
				...
				Collections.sort(vendedores, new Comparator<Vendedor>() {
					public int compare(Vendedor o1, Vendedor o2) {
						if (o1.getTotalVentas()>o2.getTotalVentas())//1 CC+3
							return -1;
						else if (o1.getTotalVentas()<o2.getTotalVentas())//1 CC+4
							return 1;
						return 0;
					}			
				});
				msj = "";
				for (Vendedor vn : vendedores) {//1 CC+3
